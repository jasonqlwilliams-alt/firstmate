...
- **Event-driven supervision** - a bash watcher sleeps on the fleet and wakes the first mate only when something needs you; verified primary harnesses also get a turn-end backstop that blocks or follows up on a blind stop when work is under way and supervision is not live.
  Claude also needs a short lease-maintenance turn about every six hours while supervision demand remains.
...
Claude Code uses a tracked Stop hook for watcher re-arm and rewake, with one short maintenance turn about every six hours while supervision demand remains, Grok uses background-notify wake cycles, and Pi uses its tracked primary watcher extension.
...
- [docs/backup-helm.md](docs/backup-helm.md) - arm the automatic Claude-runway handover to the Cursor Grok 4.6 high primary.
