...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${FM_FAKE_NO_MISTAKES_VERSION:-no-mistakes version v1.72.0 (fake) 2026-06-27T00:02:18Z}"
  exit 0
fi
exit 0
SH
...
minimum no-mistakes version is accepted^no-mistakes version v1.72.0 (fake)^empty
newer no-mistakes minor is accepted^no-mistakes version v1.73.0 (fake)^empty
newer no-mistakes major is accepted^no-mistakes version v2.0.0 (fake)^empty
older no-mistakes patch reports an upgrade^no-mistakes version v1.71.9 (fake)^missing
