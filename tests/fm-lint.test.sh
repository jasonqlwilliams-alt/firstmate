...
# `shellcheck bin/*.sh bin/backends/*.sh bin/fm-delivery-shims/* tests/*.sh`, so PRs passed local
...
test_list_files_reports_the_shell_inventory() {
  ...
  expected=$({ find bin bin/backends tests -maxdepth 1 -type f -name '*.sh' -print; printf '%s\n' bin/fm-delivery-shims/gh bin/fm-delivery-shims/gh-axi bin/fm-delivery-shims/git; } | LC_ALL=C sort)
