...
test_missing_endpoint_exit_is_already_stopped() {
  local dir out rc
  dir=$(new_case gone)
  add_task "$dir" t1 claude
  : > "$dir/fake/windows"
  out=$(run_control "$dir" t1 exit); rc=$?
  expect_code 0 "$rc" "exiting a missing endpoint should succeed as already-stopped"
  assert_contains "$out" "already-stopped t1" "the outcome should say there was no agent to stop"
  [ -z "$(literals "$dir")" ] || fail "a missing endpoint must not be sent an exit command"
  pass "fm-control exit: a missing endpoint is idempotent already-stopped success"
}
