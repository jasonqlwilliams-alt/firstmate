...
# each replace only their own exact husk. Bravo's home is a separate Firstmate
# tree, so each home treats the other's slot claims as foreign and refuses
# them. Sharing one pool would let Treehouse hand either recovery the other's
# freed slot, so bravo recovers from its own pool, as the fixtures above do.
