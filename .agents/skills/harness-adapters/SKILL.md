...
agy publishes `ANTIGRAVITY_AGENT=1`, but that name is shared with the Antigravity desktop app and IDE, so it counts only alongside anchored `agy` ancestry and is tested first because agy scrubs no inherited marker, as `references/harness/agy.md` records.
