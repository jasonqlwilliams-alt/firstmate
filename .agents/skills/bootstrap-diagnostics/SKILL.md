...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `no-mistakes`, this also covers an installed version below the `NO_MISTAKES_MIN` floor owned by [`bin/fm-bootstrap.sh`](../../../bin/fm-bootstrap.sh); treat it as a plain upgrade request, and never argue the floor down to whatever the home happens to have installed.
