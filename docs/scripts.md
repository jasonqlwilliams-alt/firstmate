...
| `fm-system-state-card.sh` | Generate the TTL 60m live SYSTEM_STATE card from probes, write `data/system-state.md`, and optionally publish it through Eleusis |
...
| `fm-urgent-alert.sh`     | Write one URGENT-ALERT packet into a configured Packet Router inbox, or no-op when this home has none |
...
| `fm-claude-stop-autoarm.sh` | Claude Stop `asyncRewake` hook owning watcher continuity with demand-gated six-hour renewal and single-flight exit-2 rewake (docs/watcher-continuity.md) |
...
| `fm-pi-system-vault-reviewer.sh` | Own the Continuum system-vault review-only Pi pin, launch flags, model refusals, and receipt JSON |
...
| `fm-backup-helm.sh`      | Arm the Claude `all_models` runway watch that hands the helm to the configured successor primary |
