...
A numeric session-lock owner that is positively absent, terminal, or not a harness under the shared `fm_harness_pid_alive` predicate is reclaimed through `bin/fm-lock.sh` before auto-arm state changes, while a stopped harness owner is terminated and verified unable to resume before replacement; a live or unclassifiable owner, absent lock, or malformed lock keeps the competing hook inert.
