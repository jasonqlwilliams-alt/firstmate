...
An opt-in `--urgent-alert` on a live hold, including a retry of an active one, may write one Packet Router URGENT-ALERT packet; `bin/fm-urgent-alert.sh` owns that emit and no-ops when the home has no inbox configured.
