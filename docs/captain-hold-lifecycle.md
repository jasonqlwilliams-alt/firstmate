...
An opt-in `--urgent-alert` on a new live hold may write one Packet Router URGENT-ALERT packet; `bin/fm-urgent-alert.sh` owns that emit and no-ops when the home has no inbox configured.
