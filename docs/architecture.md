...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits, attended captain-held transfers, and agent-less active captain backlog holds that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
An active captain backlog hold also parks a positively agent-less recorded endpoint; a live worker retains the wedge path even with a captain-held status.
An external wait (`paused:`) is the live-worker exception to that bound: the declaration identifies the long-running foreground call as a bounded wait, so it takes the `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation.
...
A declared external wait, an attended verified captain-held transfer, or an attended captain hold paired with a positively agent-less endpoint trades that silence for one bounded recheck per pause window, naming which human the wait is on; while the away-posture record exists, captain-held work waits without rechecks and remains visible in the return brief.
