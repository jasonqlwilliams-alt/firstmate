...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits, verified captain-held transfers, and agent-less active captain backlog holds that remain parked past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The secondmate gate deliberately skips backlog-only waits.
...
In normal mode, a live busy crew's own external-wait declaration (`paused:`) is the declaration exception to that bound, so it takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation.
In that normal-mode path, an active captain backlog hold takes that cadence only when the recorded endpoint is positively agent-less, so a live captain-held worker retains the busy-pane wedge path.
Lifting the external-wait declaration or losing either fact in the held-and-agent-less pair restores the unchanged busy-pane wedge path, while a pane that is no longer busy returns to the existing idle classification.
While away mode is active, a busy pane that crosses the bound under a `paused:` declaration is handed to the daemon as the plain wake identity instead of taking that recheck in the watcher, because the daemon owns triage there and a wake already decorated as a possible wedge would override the daemon's own declared-wait verdict; the held-and-agent-less pair is parked before handoff, and every other busy pane past the bound enters the wedge path before daemon classification.
That handoff is keyed on the `paused:` declaration itself (the status log's signature) rather than on the pane capture, so a harness footer that ticks on every poll wakes the daemon once per declaration instead of once per poll, and it clears the wedge timer, escalation count, and worktree-write deferral exactly as the normal-mode absorber does, so an undeclared busy phase's timer does not resume when the declaration lifts.
...
An ordinary task that the authoritative backlog currently reports with `hold_kind=captain` is also absorbed while idle in normal and away modes, but only when its recorded endpoint exists and the backend positively reports the agent state as `dead`.
Every other hold kind and every missing, live, ambiguous, unreadable, or unverified liveness result fails to qualify for this backlog-derived suppression and continues into the existing status and pane classification.
The watcher refreshes the active captain-hold set and endpoint verdict on every poll, so removing the hold or observing a live agent clears this backlog-derived suppression before the lane resumes existing classification.
A failed backlog read disables this suppression for that poll, while an empty result or malformed row proves no qualifying hold for the affected task.
...
A declared external wait, a verified captain-held transfer, or an active captain hold paired with a positively agent-less endpoint trades that silence for one bounded recheck per pause window, naming which human the wait is on, so neither a forgotten pause nor a forgotten hold can remain invisible indefinitely.
...
That block owns the live wait shape for the running primary harness: Claude's Stop `asyncRewake` hook owns re-arm cycles with one short lease-maintenance turn about every six hours while supervision demand remains, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, omp uses its own two tracked `.omp/extensions/` files, and OpenCode uses its TUI plugin.
