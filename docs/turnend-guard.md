...
`bin/fm-watch.sh` updates `state/.last-watcher-beat` once per cycle, immediately before its terminal wait (`event_wait_or_sleep`) as well as at the top of the next cycle, writing one informational line while mtime remains the liveness authority, so a healthy watcher's beacon can legitimately age up to `FM_POLL` seconds between updates.
...
- Claude registers two `Stop` hooks in `.claude/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`, and `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`.
  The arm has a six-hour lease and requests one short maintenance turn while supervision demand remains, before the native deadline kills the hook tree.
  Idle and AFK homes do not renew; [watcher continuity](watcher-continuity.md) owns the lease and acknowledgement contract.
