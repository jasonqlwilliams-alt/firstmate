...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless they declare a recognized wait or the task has both an active captain backlog hold and a positively agent-less recorded endpoint
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher re-surfaces a recognized declared wait or an active captain backlog hold paired with a positively agent-less recorded endpoint; the watcher refreshes the hold and liveness pair every poll, while the away-mode daemon separately applies the same setting to status-declared waits and ages its window against the crew's own latest status line rather than pane busy state
