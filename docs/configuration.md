...
It also sets `pr.publish_intent: false` so the generated Intent section is kept out of pull request descriptions and private routing text cannot leak to the forge.
...
agy (Antigravity CLI) is likewise verified for crewmate and scout launches ONLY and refused for a secondmate: it has no primary supervision protocol, and the captain's standing rule keeps Antigravity out of owning coding, refs, merges, and deploys.
Unlike rovo it does have a real turn-end hook, so its busy state and turn-end notification are semantic rather than a rendered-text fallback; [`docs/verification/agy.md`](verification/agy.md) owns that evidence, including the workspace-trust dialog that `--dangerously-skip-permissions` does not cover, the two-workspace launch that a single `--add-dir` would break, and the composer verdict deliberately left as `unknown`.
An agy spawn pre-registers the task worktree in agy's own trusted-workspace store before launch, the same control the claude adapter uses, because firstmate cannot answer an interactive trust dialog by key.
...
agy is the one adapter that CAPS instead of omitting: it accepts only `low`, `medium`, and `high`, and an unsupported value is a hard launch failure rather than an ignored flag, so `fm-spawn.sh` maps a requested `xhigh` or `max` onto `high` while still recording the requested value verbatim in task meta.
A profile naming it is written exactly like any other, for example `{ "harness": "agy", "model": "gemini-3.1-pro-high", "effort": "high" }`, with the model taken from `agy models` for the authenticated account.
Firstmate does not ship a rule routing work to agy: what work Antigravity may receive is the captain's standing decision, and this file documents only the profile shape.
...
The essential universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.72.0 or newer, compatible gh-axi, chrome-devtools-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless admitted directly to the declared-wait cadence, while a live idle declared wait still surfaces once before that cadence bounds repeats; an active captain backlog hold with a positively agent-less endpoint is also admitted directly
FM_BUSY_TURN_MAX_SECS=3600         # maximum age without a completed turn or explicit native-harness progress (bin/fm-watch.sh owns marker selection), before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart; a declared external wait takes the FM_PAUSE_RESURFACE_SECS recheck below instead; a captain hold requires a positively agent-less recorded endpoint
FM_PAUSE_RESURFACE_SECS=14400      # four hours between bounded rechecks of a declared external wait or verified captain-held transfer, and between repeated new-hash stale alarms for an ordinary crew task with an open backlog captain call; a structured until time can make an external-wait recheck occur sooner but cannot extend this bound; this includes a live idle pane after its first inconclusive stale wake and an externally paused busy pane past FM_BUSY_TURN_MAX_SECS, while the away-mode daemon uses the same setting and ages its window against the crew's own latest status line rather than pane busy state; a captain-held transfer is never rechecked while the away-posture record exists; active captain backlog holds paired with positively agent-less endpoints use this cadence while attended, with the pair refreshed every poll
