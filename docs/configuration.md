...
The essential universal toolchain is node, git, gh with GitHub auth via `gh auth login`, compatible no-mistakes per [CONTRIBUTING.md](../CONTRIBUTING.md), compatible gh-axi, chrome-devtools-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
