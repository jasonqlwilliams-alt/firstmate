...
That headless launch replaces the launcher shell, redirects standard streams, and, on hosts with `/proc/self/fd` such as Linux, closes other inherited file descriptors so a caller's output capture can finish once the server reports running.
