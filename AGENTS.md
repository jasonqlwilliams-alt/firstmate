...
1. **Never write to a project.**
   ...
   The only exceptions are the guarded project initialization, repository-delivery arming and upstream synchronization, fleet sync, secondmate sync and inherited local-material propagation, self-update, and approved `local-only` merge paths, each owned by its referenced skill or script, plus a concrete captain-approved project operation governed directly by this rule.
...
config/cursor-model-allowlist  spawn-time Cursor model lock; LOCAL, gitignored; absent defaults to cursor-grok-*; auto and every non-matching id are refused at launch, and dispatch profiles cannot reintroduce them; inherited by secondmate homes; see docs/configuration.md "Cursor model allowlist"
config/secondmate-harness  harness the PRIMARY uses to launch SECONDMATE agents, optionally followed by a model and effort token on the same line ("<harness> [<model>] [<effort>]"; section 4); LOCAL, gitignored; absent or "default" harness falls back to config/crew-harness then firstmate's own. The primary's own setting; NOT inherited into secondmate homes (secondmates do not spawn secondmates)
config/backup-helm-successor  successor primary for the Claude-runway backup helm, one line "<harness> [<model>] [<effort>]"; LOCAL, gitignored; absent = arm refuses with no default; see docs/backup-helm.md; not inherited
...
config/packet-router-inbox  optional absolute path to this home's Packet Router inbox/new directory for halt-progress URGENT-ALERT packets and generated SYSTEM_STATE publish; LOCAL, gitignored, and not inherited; absent means no packet is written; see docs/configuration.md "Packet Router inbox"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  system-state.md    generated live-facts overlay (TTL 60m, no model tokens); LOCAL, gitignored; owned by bin/fm-system-state-card.sh; see docs/configuration.md "Generated system-state card"
...
state/               runtime records and signals; gitignored
  ...
  .last-watcher-beat watcher liveness beacon; mtime freshness is authoritative and the one-line content is informational; updated every poll (including while absorbing benign wakes); guard scripts read mtime
...
7. **Context digest and next step** - last of the bulk sections, the full contents of `data/projects.md`, `data/secondmates.md`, `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, each clearly delimited, a one-line `SYSTEM_STATE` pointer (not the card body; `bin/fm-system-state-card.sh --pointer`), followed by the closing reminder.
...
Cursor `--model` values must match `config/cursor-model-allowlist` at spawn.
Dispatch profiles choose among allowed Grok ids and cannot reintroduce a blocked Cursor model (`docs/configuration.md` "Cursor model allowlist").
