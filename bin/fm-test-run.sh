...
family_for_basename() {
  case "$1" in
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-check-unregister.test.sh|fm-origin-repository.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|\
    ...
    fm-backup-helm.test.sh|fm-live-gate.test.sh|\
...
tests/fm-backup-helm.test.sh 15000
...
tests/fm-delivery-guard.test.sh 1314
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-procevent-quota.sh)
      printf '%s\n' "__script__:fm-procevent-quota.test.sh"
      ;;
    bin/fm-backup-helm.sh|docs/backup-helm.md)
      printf '%s\n' "__script__:fm-backup-helm.test.sh"
      ;;
