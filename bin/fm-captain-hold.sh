...
task_show() {  # <id>; sets TASK_SHOW_OUTPUT
  ...
  if [ "$status" -eq 0 ] && printf '%s\n' "$TASK_SHOW_OUTPUT" | grep -q '^task:$'; then
    return 0
  fi
  # A completed lookup may have lost a resolved row to tasks-axi's archive.
  # A timeout above must never be converted into archive evidence or absence.
  TASK_SHOW_OUTPUT=$(archived_task_show "$data" "$1")
}
