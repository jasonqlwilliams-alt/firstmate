...
# config/repository-policy.json carries the primary-authoritative upstream/fork
# and approved-owner boundary into every secondmate home. Primary
...
# Primary config/cursor-model-allowlist is the spawn-time Cursor model lock
# (default cursor-grok-* when absent), so it flows down too and a secondmate's
# own cursor crewmates cannot reintroduce a blocked Cursor model.
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget repository-policy.json trace-context launch-env-allowlist claude-permission-mode cursor-model-allowlist}"
