...
A GitHub Actions check (`Require no-mistakes`) runs on PRs targeting `main` and requires both the deterministic signature and a parseable structured attestation in the format introduced by no-mistakes v1.46.0.
...
During an active pipeline, return that failure to its outer executor to repair publication; rerunning a failed workflow re-reads the live PR body and passes only after the attestation is refreshed for the current head.
...
3. Initialize the gate with your fork as the push target: `no-mistakes init --fork-url git@github.com:<you>/firstmate.git` (contributing to firstmate requires **no-mistakes v1.72.0+**, the shared no-mistakes daemon version that Firstmate bootstrap enforces; without a fork, plain `no-mistakes init` still works for maintainers with push access).
